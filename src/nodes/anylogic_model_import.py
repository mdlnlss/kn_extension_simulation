...
class AnyLogicModelImporter:
    """Import an AnyLogic simulation model and prepare it for execution within a KNIME workflow.

    This node registers an AnyLogic `.jar`-based model and copies its files into a
    structured resource directory inside the KNIME workspace. Configuration values
    are passed downstream as flow variables.

    ### Features:
    - Validates that the model file is a `.jar` archive.
    - Supports file-based or database output modes.
    - Creates a timestamped `Resources/` directory in the KNIME workspace.
    - Exports key settings (tool, model path, output mode, output filename) as flow variables.
    """
    ...
    def execute(self, exec_context):
        ...
        selected_path = self.anylogic_model_path
        ...
        created_folder_dir = os.path.join(workspace_folder_dir, "Resources", f"ANYLOGIC_{now}")

        exec_context.flow_variables.update({
            "simulation_tool": "ANYLOGIC",
